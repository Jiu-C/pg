...
提示：如果遇到极速GO原画反复快速报错，不一定是被封号，可尝试杀掉播放器重啓，或重啓整个播放设备解决。
...
"thread_limit":32, //这里是阿里云盘的GO代理的并发协程数或java代理的并发綫程数，若遇到账号被限制并发数，请将此数值改爲10
"is_vip":true, //是否是阿里云盘的VIP用户，设置为true后，使用vip_thread_limit设置的数值来并发加速。如本设置项目不是true，则自动隐藏“转存原画”
"vip_thread_limit":10, //这里是阿里云盘的转存原画(OpenToken)并发綫程数，若遇到账号被限制并发数，请将此数值改爲10
"quark_thread_limit":32, //这里是夸克网盘GO代理的并发协程数或java代理的并发綫程数，若遇到账号被限制并发数，请将此数值改爲10
"quark_vip_thread_limit":16, //这里是夸克网盘设置quark_is_vip:true之后的并发綫程数，若遇到账号被限制并发数，请将此数值改爲10
"quark_is_vip":false, //是否是夸克网盘的VIP用户，设置为true后，綫程数受quark_vip_thread_limit控制
